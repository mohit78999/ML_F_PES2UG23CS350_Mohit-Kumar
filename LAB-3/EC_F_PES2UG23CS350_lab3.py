...
def get_entropy_of_dataset(tensor: torch.Tensor):
    ...
    entropy = -(probabilities * torch.log2(probabilities)).sum().item()
    return entropy
...
def get_avg_info_of_attribute(tensor: torch.Tensor, attribute: int):
    ...
    attribute_values, counts = torch.unique(tensor[:, attribute], return_counts=True)
    weighted_entropy_sum = 0.0
    
    for val in attribute_values:
        subset = tensor[tensor[:, attribute] == val]
        entropy_subset = get_entropy_of_dataset(subset)
        weighted_entropy_sum += (subset.size(0) / total_samples) * entropy_subset
...
def get_selected_attribute(tensor: torch.Tensor):
    ...
    """
    num_attributes = tensor.size(1) - 1  # excluding target column
    info_gains = {}
